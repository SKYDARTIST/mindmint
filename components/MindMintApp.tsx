...
import { EmptyState, LoadingState } from './CanvasStates';
...
export default function MindMintApp() {
  ...
  const [activePlaceholder, setActivePlaceholder] = useState("Paste your notes here to generate...");
  ...
  const [freeRuns, setFreeRuns] = useState(3);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (mode === AppMode.MINDMAP && !result) {
  ...
  const renderVisualOutput = () => {
    ...
    switch (mode) {
      case AppMode.MINDMAP:
        return <MermaidRenderer chart={output as string} theme={theme} />;
      ...
      case AppMode.INFOGRAPHIC:
        // Pass the current layout as activeLayout to enforce rendering of user selection
      ...
      case AppMode.SUMMARY:
        return (
          <div className="prose dark:prose-invert max-w-none text-sm leading-7 text-gray-700 dark:text-gray-300 p-4 md:p-8">
             <div className="whitespace-pre-wrap">{output as string}</div>
          </div>
        );
      default:
