...
const FlashcardViewer: React.FC<FlashcardViewerProps> = ({ cards, layout = 'classic' }) => {
      ...
      >
        <div className={`relative w-full h-full transition-all duration-500 transform-style-3d shadow-xl rounded-3xl ${isFlipped ? 'rotate-y-180' : ''}`}>
          ...
          <div className={`absolute w-full h-full backface-hidden rounded-3xl p-8 md:p-12 shadow-sm transition-colors ${styles.front}`}>
            <p className={`${styles.questionClass} overflow-y-auto max-h-[80%] no-scrollbar px-4`}>
              {currentCard.question}
            </p>

            <div className="absolute bottom-6 opacity-0 group-hover:opacity-100 transition-opacity text-xs text-gray-400 dark:text-gray-500 font-medium">
              Click to flip
            </div>
          </div>
          ...
          <div className={`absolute w-full h-full backface-hidden rotate-y-180 rounded-3xl p-8 md:p-12 shadow-sm ${styles.back}`}>
            <p className={`${styles.answerClass} overflow-y-auto max-h-full no-scrollbar px-4`}>
              {currentCard.answer}
            </p>
          </div>
