            ...
            return (
                <div className="max-w-[1800px] mx-auto py-24">
                    <div className={`p-20 rounded-[64px] border-4 ${isDark ? 'bg-[#111116]/80 border-white/5 shadow-2xl shadow-indigo-500/10' : 'bg-gray-50 border-gray-200'} relative overflow-hidden`}>
                        ...
                        <div className="relative z-10 whitespace-pre-wrap leading-[1.6] text-[48px] font-medium tracking-tight">
                            {content}
                        </div>
                    </div>
                </div>
                        ...
                        <div className="space-y-24">
                            {data.steps?.map((step: any, i: number) => (
                                <div key={i} className="flex gap-20 items-center relative z-10">
                                    ...
                                    <div className={`flex-1 p-16 rounded-[48px] border-4 ${isDark ? 'bg-[#1C1C1F] border-white/5 shadow-2xl' : 'bg-gray-50 border-gray-200'}`}>
                                        <h3 className="text-[64px] font-black mb-6 leading-none tracking-tighter uppercase">{step.title}</h3>
                                        <p className="text-[36px] text-gray-400 font-medium leading-[1.4] tracking-tight">{step.description}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        {data.steps?.map((step: any, i: number) => (
                            <div key={i} className="w-[500px] relative">
                                ...
                                <div className={`h-full p-12 rounded-[64px] border-4 ${isDark ? 'bg-gradient-to-br from-[#1C1C1F] to-[#161619] border-white/10 shadow-3xl' : 'bg-white border-gray-200'} flex flex-col items-center text-center`}>
                                    <div className="w-16 h-16 bg-indigo-500 rounded-full mb-8 shadow-[0_0_30px_rgba(99,102,241,0.5)]" />
                                    <h3 className="text-[48px] font-black mb-6 leading-tight tracking-tighter">{step.title}</h3>
                                    <p className="text-[28px] text-gray-400 font-bold leading-relaxed opacity-70 italic">{step.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                        ...
                        {data.steps?.map((step: any, i: number) => (
                            <div key={i} className={`relative overflow-hidden rounded-[80px] border-4 ${isDark ? 'bg-[#111113] border-white/5 shadow-2xl' : 'bg-white border-gray-200'} p-16 min-h-[500px] flex flex-col justify-end`}>
                                ...
                                <div className="relative z-10 space-y-8">
                                    <div className={`w-16 h-2 rounded-full ${i % 2 === 0 ? 'bg-indigo-500' : 'bg-purple-500'}`} />
                                    <h3 className="text-[64px] font-black uppercase tracking-tighter leading-none">{step.title}</h3>
                                    <p className="text-[32px] text-gray-400 font-bold leading-tight">{step.description}</p>
                                </div>
                ...
                <div className="max-w-[1800px] mx-auto py-12 space-y-20">
                    {items.map((item, i) => (
                        <div key={i} className={`p-20 rounded-[64px] border-4 ${isDark ? 'bg-white/[0.03] border-white/5 shadow-2xl backdrop-blur-3xl' : 'bg-gray-50 border-gray-200'}`}>
                            ...
                            <h3 className="text-[56px] font-bold leading-tight mb-16 tracking-tight">{item.question}</h3>

                            {item.options && (
                                <div className="grid grid-cols-1 gap-6 mb-16">
                                    {item.options.map((opt, idx) => (
                                        <div key={idx} className={`p-10 rounded-3xl border-4 ${isDark ? 'bg-white/5 border-white/5' : 'bg-white border-gray-100'} text-[36px] font-semibold flex items-center`}>
                                            <span className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center text-white mr-8 font-black text-2xl">{String.fromCharCode(65 + idx)}</span>
                                            {opt}
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="mt-12 pt-12 border-t-4 border-dashed border-white/10">
                                <div className="text-[32px] font-black text-emerald-400 mb-4 flex items-center gap-4">
                                    <div className="w-4 h-4 rounded-full bg-emerald-400 animate-pulse" />
                                    Correct: {item.correctAnswer}
                                </div>
                                <div className="text-[28px] text-gray-400 leading-relaxed font-medium italic opacity-80">
        ...
        if (appMode === 'flashcards') {
            ...
            return (
                <div className="grid grid-cols-1 gap-20 py-12 max-w-[1800px] mx-auto">
                    {cards.map((card, i) => (
                        <div key={i} className={`p-24 rounded-[80px] border-4 ${isDark ? 'bg-[#111116] border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.5)]' : 'bg-gray-50 border-gray-200'} relative overflow-hidden group`}>
                            {/* Card Accent */}
                            ...
                            <div className="flex flex-col items-center text-center">
                                ...
                                <div className="space-y-12 w-full">
                                    <div>
                                        ...
                                        <div className={`text-[64px] font-black leading-none tracking-tighter ${isDark ? 'text-white brightness-125' : 'text-gray-900'}`}>{card.question}</div>
                                    </div>

                                    <div className={`p-12 rounded-[48px] ${isDark ? 'bg-white/5' : 'bg-white'} border-2 border-white/5 shadow-inner`}>
                                        <div className="text-xl font-black text-gray-500 uppercase tracking-widest mb-4">Resolution</div>
                                        <div className={`text-[44px] font-bold leading-snug ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{card.answer}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            {isFirstPage ? (
                ...
                <div className={`relative z-10 flex justify-between items-center mb-24 border-b-8 ${isDark ? 'border-white/5' : 'border-indigo-100/50'} pb-12`}>
                    <div className="flex items-center gap-8">
                        <div className="w-8 h-8 rounded-full bg-indigo-500 shadow-[0_0_20px_rgba(99,102,241,0.5)]" />
                        <span className="text-4xl font-black text-indigo-400 uppercase tracking-[0.3em]">MindMint</span>
                    </div>
                    <div className="text-5xl font-black tracking-tight opacity-40 uppercase italic">
                        {title}
                    </div>
            ...
            <div className="relative z-10 mt-40 flex justify-between items-end border-t-8 border-white/5 pt-24 px-12 pb-8">
                ...
                <div className="flex flex-col items-end gap-6 text-right">
                    <div className="flex items-center gap-6">
                        <div className="text-3xl font-black text-gray-500 uppercase tracking-widest italic">Page</div>
                        <div className={`w-16 h-16 rounded-2xl ${isDark ? 'bg-white/10' : 'bg-gray-100'} flex items-center justify-center text-4xl font-black`}>
                            {pageNumber}
                        </div>
                        <div className="text-3xl font-bold text-gray-600">/ {totalPages}</div>
                    </div>
                    <div>
                        <div className="text-2xl font-black text-gray-600 uppercase tracking-widest mb-2">Authenticated Export</div>
                        <div className={`text-4xl font-black tracking-tighter ${isDark ? 'text-white opacity-40' : 'text-gray-900 opacity-20'}`}>MM-PRO-SS-2025</div>
                    </div>
                </div>
            </div>
        </div>
