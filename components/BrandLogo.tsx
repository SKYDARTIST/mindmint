...
// The Abstract "MindMint" Symbol (Node M) - moved outside component to avoid recreation during render
const LogoIcon = ({ w, h }: { w: number; h: number }) => (
  <svg
    width={w}
    height={h}
    viewBox="0 0 32 32"
...
const BrandLogo: React.FC<BrandLogoProps> = ({ variant = 'full', className = '', size = 'md' }) => {
  ...
  if (variant === 'icon') {
    return <div className={className}><LogoIcon w={w} h={h} /></div>;
  }
